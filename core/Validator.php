...
class Validator {
    ...
    private function validate_rule($key, $rule_str) {
        ...
        switch ($rule_name) {
            ...
            case "length"     : $valid = $this->validate_rule_length($key, $options);     break;
    ...
    private function validate_rule_length($key, $options) {
        if (count($options) !== 1 || 
        ...
        $valid = TRUE;
        $exact = $options[0];
        if (!$this->has_length($key, ["exact" => $exact])) {
            $valid = FALSE;
            $this->errors[$key] = "Please enter exactly " . $exact . " characters for " . $key;
        }
        return $valid;
    }
